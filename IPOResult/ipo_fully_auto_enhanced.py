...
class EnhancedIPOChecker:
    ...
    def __init__(
        ...
        self.saved_failed_captchas_count = (
            0  # Track how many failed captchas we've saved
        )
        self.selected_ipo = (
            None  # Store currently selected IPO (for re-selection after restart)
        )
        ...
        # Create failed captcha dataset directory if saving is enabled
    ...
    def _ensure_ipo_selected(self) -> bool:
        """
        Ensure the correct IPO is selected (used after browser restart).
        Uses the same logic as the initial selection flow.

        Returns:
            True if IPO is correctly selected or no selection needed
            False if IPO selection failed
        """
        if self.selected_ipo is None:
            self._log("No IPO stored for re-selection, assuming default is OK")
            return True  # No IPO stored, assume default is OK

        # If it's the default IPO, no need to interact with dropdown
        if self.selected_ipo.get("isDefault"):
            self._log("Selected IPO is default, no re-selection needed")
            return True

        try:
            # Re-select the IPO using stored index
            ipo_index = self.selected_ipo.get("index")
            ipo_text = self.selected_ipo.get("text", "Unknown")

            self._log(f"Re-selecting IPO: {ipo_text[:60]}... (index: {ipo_index})")
            if self.debug:
                console.print("[dim]Re-selecting IPO after page reload...[/dim]")
            ...
            self._log("Initializing dropdown by fetching IPO list...")
            ipos = self.get_available_ipos()

            if not ipos or len(ipos) == 0:
                self._log("✗ Failed to fetch IPO list during re-selection")
                console.print("[red]✗ Could not fetch IPO list for re-selection[/red]")
                return False

            self._log(f"✓ Dropdown initialized with {len(ipos)} IPOs")

            # Now verify our stored IPO still exists in the list
            stored_ipo_found = False
            for ipo in ipos:
                if ipo.get("index") == ipo_index:
                    stored_ipo_found = True
                    self._log(f"✓ Found stored IPO in list at index {ipo_index}")
                    break

            if not stored_ipo_found:
                self._log(f"⚠️  Stored IPO index {ipo_index} not found in current list")
                console.print(
                    "[yellow]⚠️  Stored IPO no longer available, using default[/yellow]"
                )
                return True  # Use default instead

            # Now select the IPO (same as initial flow)
            self._log(f"Selecting IPO at index {ipo_index}...")
            success = self.select_ipo(ipo_index)

            if success:
                self._log("✓ Successfully re-selected IPO")
                if self.debug:
                    console.print("[green]✓ IPO re-selected successfully[/green]")
            else:
                self._log("✗ Failed to re-select IPO")
                console.print("[red]✗ Failed to re-select IPO[/red]")

            return success

        except Exception as e:
            self._log(f"✗ Exception during IPO re-selection: {e}")
            console.print(f"[red]✗ Error re-selecting IPO: {e}[/red]")
            return False
    ...
    def diagnose_bot_detection(self) -> dict:
        ...
        try:
            ...
            result = {"detected": False, "reason": None, "screenshot_path": None}
            ...
            has_captcha = (
                'img[alt="captcha"]' in page_source
                or 'img[alt="Captcha"]' in page_source
            )
            ...
            if not has_form or (not has_captcha and not has_ng_select):
                result["detected"] = True
                result["reason"] = (
                    f"Missing elements: form={has_form}, captcha={has_captcha}, ng-select={has_ng_select}"
                )
        ...
        try:
            ...
            if ipos and len(ipos) > 0:
                for i, ipo in enumerate(ipos):
                    ...
                    self._log(
                        f"  {i + 1}. {ipo['text'][:60]}{'...' if len(ipo['text']) > 60 else ''}{default_marker}"
                    )
                ...
                if default_text:
                    default_ipo = {"index": 0, "text": default_text, "isDefault": True}
    ...
    def select_ipo(self, index: int) -> bool:
        ...
        try:
            ...
            if self.debug:
                console.print("    [dim]Opening dropdown...[/dim]")
            ...
            # Wait for dropdown options to actually appear (with retry logic)
            max_wait = 8  # seconds
            start_time = time.time()
            options_found = False

            while time.time() - start_time < max_wait:
                options_count = self.driver.execute_script("""
                    const options = document.querySelectorAll('.ng-dropdown-panel .ng-option');
                    return options.length;
                """)

                if options_count > index:
                    options_found = True
                    self._log(f"✓ Dropdown has {options_count} options")
                    break

                elapsed = int(time.time() - start_time)
                self._log(
                    f"Waiting for dropdown options... ({elapsed}s/{max_wait}s, found: {options_count})"
                )
                time.sleep(0.5)

            if not options_found:
                self._log(f"✗ Dropdown options did not load within {max_wait}s")
                console.print("    [yellow]⚠️  Dropdown options not loaded[/yellow]")
                return False
            ...
            if not clicked:
                ...
                console.print(
                    "    [yellow]⚠️  Browser console shows option not found[/yellow]"
                )
                return False
            ...
            time.sleep(1)
            if self.debug:
                console.print("    [dim]Closing dropdown...[/dim]")
    ...
    def check_for_rejection(self) -> bool:
        ...
        try:
            ...
            if bot_challenge:
                self._log("⚠️  Bot challenge iframe detected!")
                return True
            ...
            if any(keyword in page_text for keyword in rejection_keywords):
                self._log("⚠️  Rejection detected in page content")
                return True
    ...
    def solve_captcha_with_retry(
        ...
        for retry in range(max_retries):
            ...
            self._log(
                f"Attempting to solve captcha (attempt {retry + 1}/{max_retries})..."
            )
            prediction = self.solve_captcha_cnn()

            if prediction is None:
                self._log(
                    f"Failed to solve captcha (attempt {retry + 1}/{max_retries})"
                )
    ...
    def submit_captcha(
        self, boid: str, captcha_text: str, captcha_image: Optional[np.ndarray] = None
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            if self.check_for_rejection():
                ...
                if self.debug:
                    console.print(
                        f"[yellow]⚠️  Captcha '{captcha_text}' submitted but REQUEST WAS REJECTED by server[/yellow]"
                    )
                return False, "rejected"
            ...
            if any(error in page_text for error in captcha_error_indicators):
                ...
                # Save failed captcha for manual labeling
                if captcha_image is not None:
                    self._save_failed_captcha_image(captcha_text, captcha_image)
    ...
    def parse_result(self) -> Dict:
        ...
        try:
            ...
            self._log("Parsing result page text (first 300 chars):")
            ...
            if (
                "not allotted" in body_lower
                or "not alloted" in body_lower
                or ("sorry" in body_lower and "allot" in body_lower)
            ):
            ...
            # Check for successful allotment - MeroShare uses "Congratulation Alloted" (with typos!)
                ...
                "congratulation" in body_lower
                or ("alloted" in body_lower and "not alloted" not in body_lower)
                or ("allotted" in body_lower and "not allotted" not in body_lower)
            ):
                result["status"] = "Allotted"

                # Try to extract quantity from "Alloted quantity : <number>" or variations
                quantity_match = re.search(
                    r"(?:alloted|allotted)\s+quantity\s*:?\s*(\d+)", body_lower
                )
                ...
                result["message"] = (
                    f"Allotted {result['quantity']} shares"
                    if result["quantity"] != "?"
                    else "Allotted"
                )
            ...
            return {"status": "Error", "quantity": 0, "message": str(e)}
            ...
            try:
                ...
                if not captcha_text:
                    ...
                    if attempt < max_attempts:
                        ...
                        time.sleep(3)
                        # Re-select IPO after page reload
                        if not self._ensure_ipo_selected():
                            console.print(
                                "[yellow]⚠️  Failed to re-select IPO, continuing anyway[/yellow]"
                            )
                        time.sleep(1)
                    continue
                ...
                if success and status == "success":
                    ...
                    result["error"] = None
                    self._log(
                        f"Parsed result: {result['status']} - {result['message']}"
                    )
                    return result
                else:
                    # Something went wrong
                    self._log(
                        f"Unexpected status after submit: success={success}, status={status}"
                    )
                    if attempt < max_attempts:
                        self.driver.get(self.BASE_URL)
                        time.sleep(3)
    ...
    def check_multiple_boids(self, boids: List[str], ipo_name: str) -> List[Dict]:
        ...
        for idx, boid in enumerate(boids):
            ...
            console.print(
                f"[bold]Checking BOID {idx + 1}/{len(boids)}:[/bold] [cyan]{boid}[/cyan]"
            )
            ...
            if result.get("error") == "REJECTED":
                ...
                if self.rejection_count >= self.MAX_REJECTION_RETRIES:
                    ...
                    console.print(
                        "[yellow]MeroShare has blocked further requests.[/yellow]"
                    )
                    console.print(
                        "[dim]Please wait 15-30 minutes before trying again.[/dim]"
                    )
                    break
                ...
                # Re-select the IPO that was chosen by user
                if not self._ensure_ipo_selected():
                    console.print(
                        "[red]❌ Failed to re-select IPO after browser restart[/red]"
                    )
                    result = {
                        "status": "Error",
                        "quantity": 0,
                        "message": "Failed to re-select IPO after restart",
                        "error": "IPO_SELECTION_FAILED",
                        "boid": boid,
                    }
                    results[-1] = result
                    break  # Stop checking remaining BOIDs

                time.sleep(1.5)  # Let selection settle
...
def main():
    ...
    if args.boid:
        ...
        if not boids:
            ...
            console.print(
                "  [dim]2. Use --boid flag: python ipo_fully_auto_enhanced.py --boid 1301260001246310[/dim]"
            )
            return 1
        console.print(
    ...
    try:
        ...
        while True:
            ...
            if bot_check["detected"]:
                ...
                if bot_check["screenshot_path"]:
                    console.print(
                        f"   [dim]Screenshot saved: {bot_check['screenshot_path']}[/dim]"
                    )
                ...
                console.print(
                    "   [dim]3. Check the screenshot to see what MeroShare is showing[/dim]"
                )
                return 1
            ...
            if not ipos or len(ipos) == 0:
                ...
                if bot_check["detected"]:
                    ...
                    if bot_check["screenshot_path"]:
                        console.print(
                            f"   [dim]Screenshot: {bot_check['screenshot_path']}[/dim]"
                        )
                return 1
            ...
            if len(ipos) == 1:
                ...
                choice_idx = 0
                selected_ipo = ipos[0]
                checker.selected_ipo = (
                    selected_ipo  # Store for potential browser restarts
                )
            else:
                console.print(
                    Panel.fit(
                        "[bold cyan]Available IPOs[/bold cyan]", border_style="cyan"
                    )
                )
                for idx, ipo in enumerate(ipos):
                    default_marker = (
                        " [dim](default)[/dim]" if ipo.get("isDefault") else ""
                    )
                    console.print(
                        f"  [yellow]{idx + 1}.[/yellow] {ipo['text']}{default_marker}"
                    )
                console.print()
                ...
                if args.auto:
                    ...
                    choice_idx = 0
                    console.print(
                        f"[green]✓[/green] Auto-selected: [cyan]{ipos[0]['text']}[/cyan]"
                    )
                else:
                    ...
                    while True:
                        try:
                            choice = console.input(
                                f"[bold]Select IPO (1-{len(ipos)}):[/bold] "
                            ).strip()
                            ...
                            if 0 <= choice_idx < len(ipos):
                                ...
                                console.print(
                                    f"[yellow]Please enter a number between 1 and {len(ipos)}[/yellow]"
                                )
                ...
                if selected_ipo.get("isDefault"):
                    console.print(
                        "  [dim](Using default selection, no dropdown interaction needed)[/dim]"
                    )
                else:
                    console.print(
                        f"  [yellow](Changing selection to option {choice_idx + 1}...)[/yellow]"
                    )
                    # Use the actual DOM index from selected_ipo, not the filtered choice_idx
                    actual_dom_index = selected_ipo["index"]
                    if args.debug:
                        console.print(
                            f"  [dim][DEBUG] Clicking DOM index {actual_dom_index}[/dim]"
                        )
            ...
            try:
                continue_choice = console.input(
                    "[bold]Would you like to check the result again? (y/N):[/bold] "
                ).strip()
        ...
        if checker.save_captchas and checker.saved_captchas_count > 0:
            console.print(
                f"[green]💾 Saved {checker.saved_captchas_count} captcha(s) to training dataset[/green]"
            )

        if checker.save_failed_captchas and checker.saved_failed_captchas_count > 0:
            console.print(
                f"[yellow]📝 Saved {checker.saved_failed_captchas_count} failed captcha(s) for manual labeling[/yellow]"
            )
            console.print(
                "[dim]   Run 'python label_failed_captchas.py' to label them[/dim]"
            )
